            ...
            <div class="block-1">
                <div class="about-block">
                    ...
                    <div class="btn-container">
                        <a href="{{page('about')->url()}}"
                           class="appButton appButton-bold">{{page('about')->seo->title}}<i
                ...
                <div class="services-block">
                    <h1>{{page("ourServices","title")}}</h1>
                    <div class="mb-5 mt-5">{!! page('ourServices','body') !!}</div>
                    @foreach($page->services->chunk(3) as $ourServersChunk)
                        <ul class="flex-container mb-5">
                            @foreach($ourServersChunk as $ourServers)
                                @push('css')
                                    <style>
                                        .services-block .flex-container > .flex-item[data-id="{{$ourServers->id}}"]:after {
                                            background: url({{asset($ourServers->image)}}) no-repeat;
                                            background-size: cover;
                                        }
                                    </style>
                                @endpush
                                <li class="flex-item" data-id="{{$ourServers->id}}">
                                    <h2 class="text-uppercase text-lg-left text-center">{{$ourServers->title}}</h2>
                                    <div class="w-100 text-right">
                                        <a href="{{url($ourServers->seo->slug)}}">
                ...
                <div class="all-services-block">
                    <h1>{{page('services','title')}}</h1>
                    <p class="flex-grow-1">{{ Str::limit(strip_tags(page('services','body')), 350)}}</p>
                    <div id="carousel-all-services" class="carousel slide" data-ride="carousel">
                        <div class="carousel-inner">
                            @foreach($services->chunk(6) as $service_chunk)
                                <div class="carousel-item @if($loop->first) active @endif">
                                    <ul class="flex-container">
                                        @foreach($service_chunk as $service)
                                            @push('css')
                                                <style>
                                                    .all-services-block .flex-container > .flex-item[data-id="{{$service->id}}"]:after {
                                                        background: url({{asset($service->image)}}) no-repeat;
                                                        background-size: cover;
                                                    }
                                                </style>
                                            @endpush
                                            <li class="flex-item" data-id="{{$service->id}}">
                                                <h2 class="text-uppercase text-lg-left text-center">{{$service->title}}</h2>
                                                <div class="w-100 text-right">
                                                    <a href="{{url($service->seo->slug)}}">
                                                        <svg class="icon-right" viewBox="0 0 15 24" fill="none"
                                                             xmlns="http://www.w3.org/2000/svg">
                                                            <path
                                                                d="M13.717 11.0635L3.03666 0.383412C2.78964 0.136194 2.45988 0 2.10828 0C1.75667 0 1.42692 0.136194 1.17989 0.383412L0.393363 1.16975C-0.118438 1.68213 -0.118438 2.51491 0.393363 3.02651L9.36188 11.995L0.383412 20.9735C0.136389 21.2207 0 21.5503 0 21.9017C0 22.2535 0.136389 22.583 0.383412 22.8305L1.16994 23.6166C1.41716 23.8638 1.74672 24 2.09832 24C2.44993 24 2.77969 23.8638 3.02671 23.6166L13.717 12.9267C13.9646 12.6787 14.1006 12.3476 14.0998 11.9956C14.1006 11.6422 13.9646 11.3113 13.717 11.0635Z"
                    ...
                    <div class="d-flex justify-content-lg-start justify-content-center">
                        <a href="{{route('services')}}" class="appButton appButton-bold">все
                            услуги<i class="icon-right-white d-lg-block d-none"></i></a>
                    </div>
                </div>
                <div class="treatment-stages-block">
                    <h1>{{page("treatmentSteps","title")}}</h1>
                    <div class="body">{!! page('treatmentSteps','body') !!}</div>
                    <ul class="timeline">
                ...
                <div class="request-block">
                            ...
                            @foreach($requests as $request)
                                <div class="carousel-item @if($loop->first) active @endif">
                                    <div class="request-item">
                                        ...
                                        <a class="position-absolute" href="{{page('requests')->url()}}"> Читать
                                            польностью
                    ...
                    <div class="d-flex justify-content-start">
                        <a href="{{route('requests')}}" class="appButton appButton-bold mr-lg-5 mr-3">все Отзывы<i
                                class="icon-right-white d-lg-block d-none"></i></a>
                ...
                <div class="cases-block">
                                ...
                                <div class="carousel-item @if($loop->first) active @endif">
                                    <div class="cases-item">
                                        ...
                                        <div>
                                            ...
                                            <a class="position-absolute" href="{{url($case->seo->slug)}}"> Читать
                                                польностью
                                                <svg width="8" height="14" viewBox="0 0 8 14" fill="none"
                                                     xmlns="http://www.w3.org/2000/svg">
                                                    <path
                    ...
                    <div class="d-flex justify-content-lg-start justify-content-center">
                        <a href="{{route('cases')}}" class="appButton appButton-bold">все кейсы<i
                                class="icon-right-white d-lg-block d-none"></i></a>
                    </div>
                </div>
            </div>
